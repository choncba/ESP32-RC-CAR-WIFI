...
{ 
  ...
  if(RemoteXY.connect_flag){
    ...
    if(millis() - prev_time > 1000){
      // Battery voltage has a 2x100K divisor to avoid damage ESP32 pin over 3.3v
      // Fully charge has 4200 mV -> 2100 -> 100%
      // Battery Low has 3600 mV -> 1800 -> 0%
      uint32_t battery_read = analogReadMilliVolts(BATTERY);
      map(battery_read, 1800, 2100, 0, 100);  // 0 - 100 %
      RemoteXY.battery = int8_t(battery_read);
      RemoteXY.signal = esp_ble_tx_power_get(ESP_BLE_PWR_TYPE_DEFAULT); // Read BLE TX Power
      Serial.print("Battery: ");
      Serial.print(RemoteXY.battery);
      Serial.println(" %");
      Serial.print("BLE TX: ");
      Serial.print(RemoteXY.signal);
      Serial.println(" dB ?");
      prev_time = millis();
    }
